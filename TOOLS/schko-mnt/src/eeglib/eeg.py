...
from src.eeglib.features import (bandPower, hjorthActivity, hjorthMobility,
                             hjorthComplexity, sampEn, LZC, DFA, HFD, PFD,
                             synchronizationLikelihood)
from src.eeglib.preprocessing import bandPassFilter
from src.eeglib.auxFunctions import listType
from itertools import chain
import pyeeg as pe
# Default bands ranges
defaultBands = {"theta": (3, 8), "alpha": (8, 13), "beta": (13, 30),"gamma": (30,64)}
...
class EEG:
    ...
    def pyeeg_bin_power(self,bands=defaultBands, i=None):
        ...
        Power_ratio
            ...
            frequency bins.
        """
        X=self.window.getChannel(i)
        Fs = self.sampleRate
        count = 0
        Band = []
        for key,b in bands.items():
            Band.append(b[0])
            if count == len(bands)-1:
                Band.append(b[1])
            count += 1
        all_vals = []
        if i is None:
            for c in X:
                all_vals.append(pe.bin_power(c, Band, Fs)[0].tolist())
        else:
            all_vals = pe.bin_power(X, Band, Fs)[0].tolist()
        # X, Band, Fs
        return all_vals
    
    def pyeeg_hjorth(self,bands=defaultBands, i=None):
        X=self.window.getChannel(i)
        Fs = self.sampleRate
        count = 0
        Band = []
        ...
        if i is None:
            for c in X:
                all_vals.append(pe.hjorth(c))
        else:
            all_vals = pe.hjorth(X)
        # X, Band, Fs
        return np.array(all_vals)
    
    def pyeeg_hfd(self,bands=defaultBands, i=None):
        X=self.window.getChannel(i)
        Fs = self.sampleRate
        count = 0
        Band = []
        ...
        if i is None:
            for c in X:
                all_vals.append(pe.hfd(c,len(c)//4))
        else:
            all_vals = pe.hfd(X,len(X)//4)
        # X, Band, Fs
        return np.array(all_vals)
        
    def pyeeg_entropy(self,bands=defaultBands, i=None):
        X=self.window.getChannel(i)
        Fs = self.sampleRate
        count = 0
        Band = []
        ...
        if i is None:
            for c in X:
                all_vals.append(pe.samp_entropy(c,1, 0.2))
        else:
            all_vals = pe.samp_entropy(X,1, 0.2)
        # X, Band, Fs
        return np.array(all_vals)

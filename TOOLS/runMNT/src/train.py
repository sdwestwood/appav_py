...
"""
Sean Changes
1) removed the .long() conversion if statements)
2) removed if statements in writing at end (no valence/arousal stuff)
...
def train_model(settings, hyp_params, train_loader, valid_loader, test_loader):
    ...
    def train(model, optimizer, criterion, ctc_a2l_module, ctc_v2l_module, ctc_a2l_optimizer, ctc_v2l_optimizer, ctc_criterion):
        ...
        for i_batch, (batch_X, batch_Y, batch_META) in enumerate(train_loader):
            ...
            if ctc_criterion is not None:
                ...
                features, v2l_position = ctc_v2l_net(features)
                
                ## Compute the ctc loss
                l_len, a_len, v_len, h_len = hyp_params.l_len, hyp_params.a_len, hyp_params.v_len, hyp_params.h_len
                # Output Labels
        ...
        with torch.no_grad():
            for i_batch, (batch_X, batch_Y, batch_META) in enumerate(loader):
                ...
                if (ctc_a2l_module is not None) and (ctc_v2l_module is not None):
                    ...
                    features, _ = ctc_v2l_net(features)   # features aligned to text
    ...
    for epoch in range(1, hyp_params.num_epochs+1):
        ...
        val_loss, _, _, val_class_accuracy, val_auc = evaluate(model, ctc_a2l_module, ctc_v2l_module, criterion, test=False)
        test_loss, _, _, test_class_accuracy, test_auc = evaluate(model, ctc_a2l_module, ctc_v2l_module, criterion, test=True)
        end = time.time()
        duration = end-start
        ...
        if epoch == 1: # overwrite existing
            ...
            col_vals.append('hyp_params.dataset')
            col_headers.append(hyp_params.dataset)
            with open(f'pre_trained_models/{hyp_params.dataset_type}_{hyp_params.dataset}.csv','w') as fd:
                writer = csv.writer(fd)
                writer.writerow(col_headers)
                writer.writerow(col_vals)
        else:
            with open(f'pre_trained_models/{hyp_params.dataset_type}_{hyp_params.dataset}.csv','a') as fd:
                writer = csv.writer(fd)
                writer.writerow(col_vals)


        if val_loss < best_valid:
            print(f"Saved model at pre_trained_models/{hyp_params.dataset_type}_{hyp_params.dataset}.pt!")
            save_model(hyp_params, model, name=hyp_params.dataset_type+'_'+hyp_params.dataset)
            best_valid = val_loss
            early_stopping = 0
        else:
            if early_stopping==10:
                print(f"Saved model at pre_trained_models/{hyp_params.dataset_type}_{hyp_params.dataset}_early_stopped.pt!")
                save_model(hyp_params, model, name=f"{hyp_params.dataset_type}_{hyp_params.dataset}_early_stopped")
            early_stopping += 1
